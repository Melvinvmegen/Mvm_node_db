...
exports.createCrypto = async (req, res, next) => {
  ...
  try {
    ...
    const uniqueTransactions = req.body.Transactions.filter((v, i, a) => a.indexOf(v) === i)
    crypto = await Crypto.create({
      name: req.body.name,
      category: req.body.category,
      pricePurchase: uniqueTransactions.reduce((sum, item) => sum + item, 0) / uniqueTransactions.length,
...
exports.updateCrypto = async (req, res, next) => {
  ...
  try {
    ...
    const fetchedCrypto = response.data.data.filter((element) => element.name === req.body.name)
    const values = fetchedCrypto[0].quote.EUR
    const uniqueTransactions = [...new Set(req.body.Transactions.map(transaction => transaction.price))]
    ...
    crypto.pricePurchase = uniqueTransactions.reduce((sum, item) => sum + item, 0) / uniqueTransactions.length
